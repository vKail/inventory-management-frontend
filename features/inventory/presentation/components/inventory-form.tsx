...
export const InventoryForm = ({ initialData, mode = 'create' }: InventoryFormProps) => {
    ...
    setTimeout(() => {
      ...
      if (!errorElement) {
        const allInputs = document.querySelectorAll(
          'input, select, textarea, [role="combobox"], [data-radix-trigger]'
        );
        Array.from(allInputs).forEach(input => {
          if (errorElement) return;
          const inputElement = input as HTMLElement;
          if (
            inputElement.getAttribute('name') === firstErrorField ||
      ...
      if (!errorElement) {
        ...
        const comboboxTriggers = document.querySelectorAll(
          '[role="combobox"], [data-radix-trigger]'
        );
  ...
  useEffect(() => {
    if (mode === 'edit' && initialData?.id) {
      ...
      ItemMaterialService.getInstance()
        .getItemMaterials(initialData.id)
      ...
      // Cargar colores
      ItemColorService.getInstance()
        .getItemColors(initialData.id)
        .then(colors => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValid) {
      ...
      if (errorCount > 0) {
        toast.error(
          `Por favor, corrija ${errorCount} error${errorCount > 1 ? 'es' : ''} en el formulario`
        );
      }
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        try {
          ...
          const existingMaterials = await ItemMaterialService.getInstance().getItemMaterials(
            initialData.id
          );
          ...
          for (const existingMaterial of existingMaterials) {
            if (
              !selectedMaterials.find(
                (m: ItemMaterial) => m.materialId === existingMaterial.materialId
              )
            ) {
          ...
          for (const material of selectedMaterials) {
            const existingMaterial = existingMaterials.find(
              (m: ItemMaterial) => m.materialId === material.materialId
            );
            if (existingMaterial) {
          ...
          for (const color of selectedColors) {
            const existingColor = existingColors.find(
              (c: ItemColor) => c.colorId === color.colorId
            );
            if (existingColor) {
  ...
  const handleBackendErrors = (response: any) => {
    if (response.message?.content && Array.isArray(response.message.content)) {
      ...
      errorMessages.forEach((errorMessage: string) => {
        ...
        for (const [backendField, formField] of Object.entries(errorFieldMap)) {
          if (
            errorMessage.toLowerCase().includes(backendField.toLowerCase()) ||
            errorMessage
              .toLowerCase()
              .includes(backendField.replace(/([A-Z])/g, ' $1').toLowerCase())
          ) {
            form.setError(formField as any, {
              type: 'manual',
              message: errorMessage,
